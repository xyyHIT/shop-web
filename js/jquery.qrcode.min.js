(function(r) {
	r.fn.qrcode = function(h) {
		...
		return this.each(function() {
			...
			//base64位转图片
			$.ajax({
				url: host+'/yjpai/platform/product/getImgUrlByStr',
				type: "post",
				data: {"entityId":erweimaId,"category":category,"imgStr":base64[0].substr(23)},
				dataType: "json",
				success: function(rs) {
					erweima1 = rs.data;
					if (rs.resCode == 1) {
						datadata = ['/yjpai/imgs/test/shareerweimabg2.png',erweima1];
						...
						function draw(fn) {
							...
							function drawing(n) {
								if (n < len) {
									...
									img.onload = function() {
										if(n == 0){
											ctx.drawImage(img, 0, 526, 640, 482);
										}else if(n == 1){
											ctx.drawImage(img, 200, 200, 240, 240);
										}
										drawing(n + 1); //递归
									}
								} else {
									...
									ctx.font = '35px arial';
									ctx.fillStyle = '#000';
									ctx.fillText(erweimaName, c1.width/2, 120);
									
									ctx.font = '26px arial';
									ctx.fillStyle = '#000';
									ctx.fillText(erweimaAddress, c1.width/2, 170);
									
									ctx.font = '26px arial';
									ctx.fillStyle = '#000';
									ctx.fillText(erweimaText1, c1.width/2, 836);
									//保存生成作品图片
									base64.push(c1.toDataURL("image/jpeg", 0.8));
						...
						draw(function() {
							$.ajax({
								url: host+'/yjpai/platform/product/getImgUrlByStr',
								type: "post",
								data: {"entityId":erweimaId,"category":category,"imgStr":base64[1].substr(23),"flag":1},//flag:最后生成二维码的标示
								dataType: "json",
								success: function(rs) {
									allFun.removeLoading();
									if (rs.resCode == 1) {
										$(".div_erweima").html('<img src="' + rs.data + '">');
										var erweimaimgList = [];
